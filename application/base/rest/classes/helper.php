<?php
/**
 * Midrub Base Rest Helper
 *
 * This file contains the class Helper
 * with methods to prepare the REST's calls
 *
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.7.9
...
/*
 * Helper has methods to prepare the REST's calls
 * 
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.7.9
...
class Helper {
    ...
    public function verify_token($permissions) {
        ...
        $token = $this->CI->oauth2->check_token($access_token);
        ...
        $token_permissions = $this->CI->oauth2->get_token_permissions($token[0]->token_id);
        ...
        if ( $token_permissions ) {
            ...
            if ( $count < count($permissions) ) {
                ...
                if($this->CI->session->has_userdata('user_id')) {
                    $this->CI->user_id = $this->CI->session->userdata('user_id');
