...
class MidrubBase {
    ...
    public function __construct($category, $static_slug=NULL, $dynamic_slug=NULL) {
        ...
        /*---==================== CLIENT LINK ====================--->*/
        /**
         * for initiate client link, not login Midrub
         */
        } else {

            // added by Jackson
            $user_id = !is_null($this->CI->input->get('id')) ? $this->CI->input->get('id') : '';
            $network = !is_null($this->CI->input->get('n')) ? $this->CI->input->get('n') : '';
            $group = !is_null($this->CI->input->get('g')) ? $this->CI->input->get('g') : '';

            $client_login = (($user_id != '') && ($network != '') && ($group != '')) ? true : false;

            if($client_login) {
                $this->CI->user_id = $user_id;
                $this->CI->group_id = $group;
                $this->CI->network = $network;
