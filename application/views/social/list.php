...
<body class="d-flex flex-column h-100">
    <main role="main" class="flex-shrink-0">
        <div class="container-fluid">
            <?php echo form_open($callback, array('data-csrf' => $this->security->get_csrf_token_name(), 'class' => 'social_blog_frm')) ?>
            <div class="jumbotron">
                <div class="row">
                    <div class="col-sm-6">
                        <h4 class="mb-0">
                    ...
                    <div class="col-sm-6">
                        <div class="row">
                            <div class="col-6">
                                <span class="total-selected-items"><?php echo $connected_accounts; ?></span>
                                <?php echo $connect; ?>
                            </div>
                            <div class="col-6 text-right">
                                <span class="total-allowed-items"><?php echo $network_accounts; ?></span>
                                <?php echo $connect; ?>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-12">
                                <div class="progress">
                                    <div class="progress-bar bg-success"
                                        style="width: <?php echo (100 - (($network_accounts - $connected_accounts) / ($network_accounts)) * 100); ?>%;"
                                        role="progressbar" aria-valuemin="0"
                                        aria-valuenow="<?php echo (100 - (($network_accounts - $connected_accounts) / ($network_accounts)) * 100); ?>"
                            ...
                            <div class="row">
                                ...
                                <div class="col-3">
                                    <div class="checkbox-option">
                                        <input id="item-<?php echo $item['net_id']; ?>" name="net_ids[]" type="checkbox"
                                            value="<?php echo $item['net_id']; ?>"
                                            <?php echo ($item['connected'])?' checked':''; ?>>
            ...
            <div class="jumbotron-footer" data-connected-accounts="<?php echo $connected_accounts; ?>"
                data-total-accounts="<?php echo $network_accounts; ?>">
                <div class="row">
                    <div class="col-sm-6">
                        <h6 class="mb-0">
                            ...
                            <span class="selected-items"><?php echo $connected_accounts; ?></span>
                            <?php echo $this->lang->line('networks_selected'); ?> <?php echo ucfirst($connect); ?>
                        </h6>
                    </div>
                    <div class="col-sm-6">
                        <div class="row">
                            <div class="col-12 text-right">
                                <button type="button" class="btn btn-info btn-sm save-btn">
                                    <svg class="bi bi-box-arrow-in-up-right" width="1em" height="1em"
...
$('.checkbox-option').click(function(e) {
    ...
    setTimeout(function() {
        ...
        $('.jumbotron-footer .selected-items').text($('input:checkbox:checked').length + parseInt($(
            '.jumbotron-footer').attr('data-connected-accounts')));

        var percentage = (100 - (((parseInt($('.jumbotron-footer').attr('data-total-accounts')) - ($(
            'input:checkbox:checked').length + parseInt($('.jumbotron-footer')
            .attr('data-connected-accounts')))) / (parseInt($('.jumbotron-footer').attr(
            'data-total-accounts')))) * 100));

        $('.progress-bar').css('width', percentage + '%');
        $('.progress-bar').attr('aria-valuenow', percentage);

        $('.total-selected-items').text($('input:checkbox:checked').length + parseInt($(
            '.jumbotron-footer').attr('data-connected-accounts')));
...
if (parseInt($('.jumbotron-footer').attr('data-total-accounts')) > 0) {
    ...
    var connected = (parseInt($('.jumbotron-footer').attr('data-connected-accounts')) - $('input:checkbox:checked')
        .length);
...
$('.save-btn').on('click', function() {
    ...
    $('input:checkbox').each(function() {
        if ($(this).is(':checked')) {
            net_ids.push($(this).val());
        }
    });

    $.ajax({
        url: url + 'user/ajax_save_net_ids',
        data: {
            net_ids: net_ids
        },
        dataType: 'json',
